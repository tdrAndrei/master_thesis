...
Dependent type systems, as introduced by Martin-L{\"o}f \autocite{martinlof1984intuitionistic}, can address this challenge. In a dependently typed language, types may depend on values, enabling programmers to express and verify arbitrary invariants within the type system itself. Moreover, the ability to bind arbitrary values within a type induces the need for type level computation (reduction). In the following sections we will rely on Agda \autocite{norell2007practical} for our examples, since it is an easy to read language that implements Martin-L{\"o}f's theory of dependent types.
...
$B$ is fixed and cannot refer to the value of the argument. A \emph{dependent
  function type} (or \emph{Pi type}), written $\Pi\,(x : A).\, B(x)$ generalizes this with the return type $B(x)$ being able to mention the argument $x$. When the return type does not depend on the argument, the Pi type reduces to an ordinary function type $A \to B$. As a first example, consider a function \texttt{default} : $\Pi\,(A : \text{Type}).\, A$
...
function type in simply typed calculus. 

In practice, many type arguments can be inferred from context. Most dependently typed languages allow such arguments to be marked as \emph{implicit}, meaning they are automatically filled in by the type checker. The canonical example is the identity function, which can be given the type $\text{id} : \{A : \text{Type}\} \to A \to A$, where the braces indicate that $A$ is implicit. The caller writes $\text{id}\; 42$ rather than
...
One of the most important feature of dependent type theory, that allows us to use values (or any terms for that matter) at compile time is the particular name binding structure. In the previous example, \texttt{unwrap} can use the \textbf{value} of its first argument in the type signature, by binding it to the name $m$.
...
In the literature, there are many types of name binding, but this work will focus on static name binding (binding that happens at compile time). In static analysis of programs it is common to infer information about a name when it is first introduced (\textit{declared}), while later \textit{references} of the identifier can re-use the collected information or refine it \autocite{4bf44aa1779c4a968c555e1b54e16119}. For well-formed program all references must be \textit{bound} to a single declaration. However, in many programming languages used in practice, it is not always obvious how to map a reference to its declaration. This process is known as \textit{name resolution} \autocite{4bf44aa1779c4a968c555e1b54e16119}.

A common way to represent the result of name resolution is through \emph{def-use chains}, which link each reference directly to its corresponding declaration \autocite{aho_compilers_2006}. Def-use analysis can be run once and its result can be reused by the compiler for different passes. Def-use chains are the backbone of type checking and numerous IDE services such as go-to-definition, find references, and rename refactoring.

The rules that govern name resolution are determined by the scoping discipline of the language. The most common discipline is \emph{lexical scoping}, where the scope of a name is determined by its position in the AST. In a lexically scoped language, an inner scope may \emph{shadow} a declaration from an enclosing scope, and resolution proceeds outward from the reference site until a matching declaration is found. Lexical scoping was popularized by Algol 60 and is used by the majority of modern programming languages \autocite{scott_programming_2015}. However, many practical languages also feature \emph{non-lexical} scoping mechanisms that cannot be explained solely by the AST hierarchy. Examples include module imports, class inheritance, qualified names, and forward-references. Non-lexical scoping increases the complexity of type checking. Since the scoping rules don't follow the hierarchy of the AST neither can the type checker. For instance, to type check forward references, the type checker cannot simply follow the AST, as it will encounter references of names that have not yet been declared. To account for this it should collect and process all available declarations within a scope before proceeding. It follows that scoping and name resolution are tightly coupled with the operational semantics of a type checker. In most languages, non-lexical scoping is handled in an ad-hoc manner embedded in the type checker and other static analysis. \autocite{neron_extensible_2015}.

\section{Name binding in dependent types}
A dependently typed language adds subtle complexities to the name binding and resolution process. One difference to a simply typed language is the ability to bind values (of any type) in types. When performing an application $f \ x$, the value of $x$ might be bound both by $f$'s binder and by $f$'s type. Another challenge of name binding comes with inference. In dependently typed languages inference is complex and requires higher-order unification as shown by \autocite{mazzoli2016typechecking,demoura2015elaboration,abel_extensions_2018}. This complexity arises from the presence of binders at the type level. In particular, when unifying two types, the solution can depend on these binders and will generally have the form of a function. More details about this process and how it is integrated in type checking, are given in Chapter \ref{chap:hou}. 

The most common approach to add inference is to create "holes" (metavariables) in places where a term must be inferred. Here, term refers to either a value or a type, since in this type system we should infer both. A familiar example is Hindley-Milner type inference, where fresh type variables represent unknown monotypes and first-order unification progressively determines their solutions. For every typeable expression, the Hindley-Milner inference algorithm computes a principal type scheme \autocite{milner1978theory,damas1982principal}. Unlike ordinary bound variables, metavariables are initially unsolved and carry only the information required to ensure that they are well-scoped and well-typed, such as their type or kind and the context in which they were introduced. As elaboration proceeds, uses of expressions containing metavariables generate unification constraints. Solving these constraints assigns \emph{solutions} to the metavariables, progressively refining the partial term until the missing information has been reconstructed \autocite{gundry2010typeinference,dunfield2013complete}.
...
\section{Another way to model name binding: Higher-Order Abstract Syntax}

An alternative approach to modeling name binding is given by Higher-Order Abstract Syntax (HOAS), in which binding constructs of an object language are represented using the binding mechanisms of a meta-language (host language). Instead of representing scopes, declarations, and references explicitly, HOAS encodes binding structure directly using functions of the meta-language. The key idea is to represent object-language binders as meta-level functions. A bound variable is represented by a meta-level variable introduced by a function argument, and occurrences of that variable are represented by the same meta-level variable. As a result, substitution and $\alpha$-equivalence are handled implicitly by the meta-language, without requiring explicit definitions of these operations.

For example, a $\lambda$-abstraction in the object language can be represented as a meta-level function wrapped in \texttt{Lam} constructor:

\begin{align*}
[\![\lambda x.\ body]\!] \Rightarrow \mathsf{Lam}(\mathsf{fun}\ x \Rightarrow body) 
\end{align*}
...
\begin{align*}
[\![f\ x]\!] \Rightarrow \mathsf{App}(f, x)
\end{align*}


In theory HOAS can be embedded in a variety of meta-languages, such as Agda or Coq, where binders are represented using higher-order functions. However, these systems rely on an intensional (or weak) function space, in which
functions are treated as opaque values that cannot be inspected or decomposed by
pattern matching. Moreover, when doing HOAS in a general purpose language, it is possible to write non-normalizing terms which the type checker will accept\footnote{Unless the datatype of terms of the object language isn't positive, but in that case the program won't compile at all.}. These problems can be reduced by implementing a variant of HOAS called PHOAS\footnote{Parametric Higher Order Syntax}\autocite{PHOAS}, but which sacrifices a lot of the convenience of working with HOAS terms. 

For this purpose, logic programming languages are easier to use with HOAS. $\lambda$-Prolog and its derivations use the so-called $\lambda$-tree syntax \autocite{lambda-tree}, in which the user can define lambda terms, quantify variables and where the runtime manages fresh variables through eigenvariables. Contrasting to general purpose languages, the semantics of a logic programming language are slightly different. Programs are expressed as relations rather than functions, and evaluation proceeds by attempting to prove goals using unification and/or backtracking. To make the discussion more concrete, we will focus on an actual implementation of these ideas. ELPI \autocite{dunchev_elpi_2015} is an embeddable implementation of $\lambda$-Prolog that adopts the $\lambda$-tree approach. It uses higher-order pattern unification on meta-level representations of object-language terms, including those containing binders. Unification is restricted to the higher-order pattern fragment, where meta-variables may only be applied to distinct bound variables, to preserve decidability. A typical use case in ELPI is the representation of typing rules. For example, the typing rule for lambda abstraction can be encoded as:
...
In this encoding, $x$ is introduced as a fresh meta-level variable, by the use of the quantifier \texttt{pi} (for-all). The implication operator \texttt{=>} extends the context with the assumption that $x$ has type $A$ when checking the body. The meta-language ensures that $x$ is correctly scoped, and that occurrences of $x$ in $Body$ refer to the same entity. Moreover, application can be expressed as:
...
This representation contrasts with first-order encodings, where explicit environments or name management must be introduced. In HOAS, these aspects are delegated to the meta-language. Therefore, the difference in how binding structures can be inspected and manipulated greatly affects the design of type checkers and unification algorithms.
